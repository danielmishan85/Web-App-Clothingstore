  ...
  switch (action.type) {
    case "ADD_TO_BASKET":
      const index2 = state.basket.findIndex(
        (basketItem) => basketItem.productName === action.productName
      );
    ...
    case "MINUS_ONE_FROM_BASKET":
      ...
      if (index3 >= 0 && state.basket[index3].amount > 1) {
        state.basket[index3].amount = state.basket[index3].amount - 1;
        return {
          ...state,
        };
      } else {
        console.warn(
          `Can't remove product(title: ${action.productName}) as its not in the basket!`
        );
      }
    default:
      return state;
